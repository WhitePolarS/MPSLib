...
import numpy as np
from entity.SNESIM import SNESIM
...
class SNESIMTree(SNESIM):
    ...
    def initialize(self, configuration_file):
        ...
        for i in range(0, len(hd_list), 4):
            ...
            print(points)
            print(points[i % 4+2], points[i % 4], points[i % 4+2], points[i % 4+3])
            self._hdg[points[i % 4 + 2]][points[i % 4]][points[i % 4 + 1]] = points[i % 4 + 3]
                ...
                while len(nodes_to_check) > 0:
                    ...
                    for i in range(0, len(current_tree_node)):
                        if is_nan(a_partial_template[current_tree_node[i].level - 1]):
                            ...
                            # 模板点匹配上了数结点，如果他的子节点counter仍不为0，那说明还有值，就继续向下搜索
    ...
    def _InitStartSimulationEachMultipleGrid(self, level):
        ...
        print("11:数据模板的大小为{} {} {}".format(template_x, template_y, template_z))
        # 构建模板结构
